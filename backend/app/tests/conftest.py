...
def enforce_test_isolation_contract() -> None:
    ...
    if app_env != "test":
        raise RuntimeError("Backend tests require APP_ENV=test. Refusing to run outside test isolation.")
    ...
    if redis_host != "redis_test":
        raise RuntimeError(f"Unsafe REDIS_URL host for tests: expected 'redis_test', got '{redis_host or '(empty)'}'.")
