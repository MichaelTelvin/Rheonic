...
from app.dependencies import get_current_user, get_ingest_key_service
...
def list_project_keys(
    ...
    try:
        return [_summary(key) for key in service.list_keys(project_id=project_id, user_id=current_user.id)]
...
def create_project_key(
    ...
    try:
        key, plaintext = service.create_key(project_id=project_id, name=payload.name, user_id=current_user.id)
        return CreateKeyOut(
            key=plaintext,
            key_id=key.id,
            name=key.name,
...
def revoke_key(
    ...
    try:
        key = service.revoke_key(key_id=key_id, user_id=current_user.id)
        return _summary(key)
    except HTTPException:
        raise
    except Exception:
...
def rotate_key(
    ...
    try:
        key, plaintext = service.rotate_key(key_id=key_id, user_id=current_user.id)
        return CreateKeyOut(
            key=plaintext,
            key_id=key.id,
            name=key.name,
