...
class TokSpikeDetector(Detector):
    # Emits token-spike signal when early absolute or gated relative thresholds are hit.
    ...
    def detect(self, ctx: DetectionContext) -> list[Signal]:
        ...
        if early_abs_hit and early_abs_evidence.get("tok_hit", False):
            return [_signal(ctx, req_ratio, tok_ratio, req_delta, tok_delta, True, early_abs_evidence)]
        if not ctx.gate.allow_baseline_relative_checks() or not tok_trigger:
            return []
...
def _signal(
    ...
    return Signal(
        ...
        fingerprint=_build_signal_fingerprint(ctx, "tok_spike"),
        evidence=evidence,
        tags=tags,
    )
