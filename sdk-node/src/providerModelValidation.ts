export class LLMTBGValidationError extends Error {
  ...
  public constructor(message: string, provider: string, model: string, expectedProviders: readonly string[]) {
    super(message);
    this.name = "LLMTBGValidationError";
...
export function validateProviderModel(provider: string, model: string | null | undefined): void {
  ...
  if (!normalizedProvider) {
    throw new LLMTBGValidationError(
      "LLMTBG: provider must be explicitly provided.",
      provider,
      String(model ?? ""),
      SUPPORTED_PROVIDERS,
    );
  ...
  if (!SUPPORTED_PROVIDERS.includes(normalizedProvider as (typeof SUPPORTED_PROVIDERS)[number])) {
    throw new LLMTBGValidationError(
      `LLMTBG: unsupported provider: ${provider}`,
      provider,
      String(model ?? ""),
      SUPPORTED_PROVIDERS,
    );
  ...
  if (!normalizedModel) {
    throw new LLMTBGValidationError(
      `LLMTBG: model must be explicitly provided for provider ${normalizedProvider}.`,
      normalizedProvider,
      String(model ?? ""),
      SUPPORTED_PROVIDERS,
    );
