...
test-backend:
	...
	@bash -lc "set -euo pipefail; trap 'docker compose -p llmtbg_test -f docker-compose.test.yml down -v >/dev/null 2>&1 || true' EXIT; docker compose -p llmtbg_test -f docker-compose.test.yml up -d postgres_test redis_test backend_test >/dev/null; docker compose -p llmtbg_test -f docker-compose.test.yml run --rm -v \"$$(pwd):/workspace\" backend_test sh -lc \"cd /workspace/backend && pytest -v\" 2>&1 | sed '/^ Container /d;/^\\[+\\]/d' | awk '{if (/^[^ ]+::[^ ]+/) sub(/^([^ ]+::[^ ]+)/, \"\\033[36m&\\033[0m\"); gsub(/PASSED/, \"\\033[32mPASSED\\033[0m\"); gsub(/FAILED/, \"\\033[31mFAILED\\033[0m\"); gsub(/ERROR/, \"\\033[31mERROR\\033[0m\"); print}'"
