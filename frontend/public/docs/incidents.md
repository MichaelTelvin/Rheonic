...
- `token_explosion`: request-context size is large, near a cap-driven threshold, or growing sharply once it has already become meaningfully large. Rheonic evaluates the same request-side signal in protect and observe so warn decisions and incidents stay aligned. By default, growth-only detection is ignored until the current request-context reaches `1800`, and `growth_count=2` means `spike + confirmation`, for example `1100 -> 1900 -> 3300`. Growth-only detection is suppressed when traffic looks highly concurrent.
